...
func (s *ChatService) SendMessage(ctx context.Context, sessionID, content string) (string, *model.ChatMessage, *model.ChatMessage, []map[string]interface{}, error) {
	...
	if len(historyMessages) > 0 {
		...
		// MCP 工具集成 - 根据Agent配置加载MCP
		if s.mcpSvc != nil && selectedAgent != nil {
			serverIDs, err := s.parseAgentMCPServerIDs(selectedAgent)
			if err != nil {
				logger.Error(fmt.Sprintf("解析Agent MCP配置失败: %v", err))
				return "", nil, nil, nil, fmt.Errorf("解析Agent MCP配置失败: %w", err)
			}

			if len(serverIDs) > 0 {
				logger.Info(fmt.Sprintf("从Agent %s加载MCP工具，共%d个服务器: %v", selectedAgent.Name, len(serverIDs), serverIDs))

				mcpTools, err := s.mcpSvc.GetToolsByServerIDs(ctx, serverIDs)
				if err != nil {
					logger.Error(fmt.Sprintf("获取Agent MCP工具失败: %v", err))
					return "", nil, nil, nil, fmt.Errorf("获取Agent MCP工具失败: %w", err)
				...
				logger.Info(fmt.Sprintf("Agent %s 未配置MCP Server，不加载MCP工具", selectedAgent.Name))
			}
		} else if s.mcpSvc != nil && selectedAgent == nil {
			logger.Info("未选择Agent，不加载MCP工具")
		}
...
// StreamSendMessageWithEvents 发送消息并返回AgentEvent流（用于OpenAI格式转换）
// 参数：enableThinking - 是否启用思考过程显示
func (s *ChatService) StreamSendMessageWithEvents(ctx context.Context, sessionID, content string, enableThinking bool) (<-chan *model.AgentEvent, *model.ChatMessage, []map[string]interface{}, error) {
	...
	// 如果启用思考过程，使用新的流式方法
	if enableThinking {
		return s.streamWithThinkingEvents(ctx, sessionID, content, eventChan)
	}

	// 否则使用原有方法
	streamChan, userMsg, ragRefs, err := s.StreamSendMessage(ctx, sessionID, content)
	if err != nil {
		close(eventChan)
		return nil, nil, nil, err
	}

	// 启动goroutine转换string stream为AgentEvent
	go func() {
		defer close(eventChan)

		agentName := "AIAssistant"

		// 发送用户消息事件
		if userMsg != nil {
			eventChan <- model.NewUserMessageEvent(sessionID, "user", userMsg.Content)
		}

		// 从streamChan接收并转换为ContentChunk事件
		for chunk := range streamChan {
			eventChan <- model.NewContentChunkEvent(agentName, chunk)
		}
	...
	return eventChan, userMsg, ragRefs, nil
}

// streamWithThinkingEvents 支持思考过程的流式消息发送
func (s *ChatService) streamWithThinkingEvents(ctx context.Context, sessionID, content string, eventChan chan *model.AgentEvent) (<-chan *model.AgentEvent, *model.ChatMessage, []map[string]interface{}, error) {
	logger.Info(fmt.Sprintf("=== StreamWithThinkingEvents START: session=%s, thinkingEnabled=true ===", sessionID))
	...
	// 使用支持思考过程的流式生成
	streamChan, err := s.llm.StreamGenerateWithReasoning(ctx, fullPrompt)
	if err != nil {
		close(eventChan)
		return nil, nil, nil, fmt.Errorf("LLM流式生成失败: %w", err)
	}

	// 启动goroutine处理流式结果并发送事件
	go func() {
		defer close(eventChan)
		...
		for result := range streamChan {
			...
			if result.ReasoningContent != "" {
				...
				logger.Info(fmt.Sprintf("发送thinking事件: %d chars", len(result.ReasoningContent)))
			}
		...
		// 保存AI消息到数据库
		aiMsg, err := s.SaveAIMessage(sessionID, fullContent.String(), ragReferences)
		if err != nil {
			logger.Error(fmt.Sprintf("保存AI消息失败: %v", err))
		} else {
			logger.Info(fmt.Sprintf("AI消息已保存: ID=%s, ContentLen=%d, ReasoningLen=%d",
				aiMsg.ID, fullContent.Len(), fullReasoning.Len()))
		}
	...
	return eventChan, userMessage, ragReferences, nil
}
...
func (s *ChatService) parseAgentMCPServerIDs(agent *model.Agent) ([]string, error) {
	if agent == nil {
		return nil, fmt.Errorf("Agent为空")
	}

	if agent.MCPServerIDs == "" {
		return []string{}, nil
	}

	var serverIDs []string
	if err := json.Unmarshal([]byte(agent.MCPServerIDs), &serverIDs); err != nil {
		return nil, fmt.Errorf("解析MCP Server IDs失败: %w", err)
	}
